...
async def on_message(ctx):
    if ctx.content.startswith("!병종입력"):
        try:
            ...
            pic1 = ctx.content.split(" ")[1]
            worksheet_army.update_cell(Guild_member.row, 2, pic1)
            pic2 = ctx.content.split(" ")[2]
            worksheet_army.update_cell(Guild_member.row, 3, pic2)
            pic3 = ctx.content.split(" ")[3]
            worksheet_army.update_cell(Guild_member.row, 4, pic3)
    ...
    if ctx.content.startswith ("!흥보시작"):
        if ctx.guild:
            if ctx.author.guild_permissions.manage_messages:
                await ctx.channel.send(f'지금부터 영토전 흥보를 시작함 13시-23시 1시간간격')
    ...
    if ctx.content.startswith ("!도움"):
        await ctx.send('!영토전참가 or !영토전참가 [이름]\n!영토전불참 or !영토전불참 [이름]\n !병종입력 [병종1] [병종2] [병종3] | 영토전에 가져오는 병종입력')

    if ctx.content.startswith ("!서버도움"):
        await ctx.send('!흥보시작 | 13시-23시 1시간간격 메시지 보냄\n!흥보메시지 [메시지] | 흥보문구 변경가능 아마도...')
